...
#+begin_example
  ...
  # -*- mode:org; epa-file-encrypt-to: ("cooper@oscarfono.com") -*-
...
#+begin_src elisp :tangle sod.el
  ...
  (setq org-capture-templates
        `(("c" "Contact" entry (file+headline "~/capture/contacts.org" "Contacts"), my/org-contacts-template :empty-lines 1)
          ("d" "Documentation" entry (file+headline "~/Documents/docs.org" "Documentation") "** %^{Subject}\n %^g\n %?\n %i\n Added %U")
          ("D" "Definition" entry (file+headline "~/capture/definitions.org" "Definitions") "** %^{Term} :: %^{Definition} ")
          ("e" "Expense" entry (file+olp+datetree "~/capture/expenses.org"), my/org-expenses-template :empty-lines 1)
          ("i" "Idea" entry (file+olp+datetree "~/capture/ideas.org" "Ideas") "**  %?\n I had this idea on %U\n %a" :empty-lines 1)
          ("j" "Journal" entry (file+olp+datetree "~/capture/journal.org") "*  %?\n Entered on %U\n" :empty-lines 1)
          ("l" "Lyric" entry (file+headline "~/capture/lyrical-ideas.org" "Lyrical Ideas Capture") "** %^{working-title}\n %^{verse}\n %^{hook}\n")
          ("p" "Project" entry (file+olp+datetree "~/Public/projects/oscarfono/projects/current-projects.org"), my/org-project-template :empty-lines 1)
          ("Q" "Quote" entry (plain "~/capture/quotes.org"), my/org-greatquotes-template :empty-lines 1)
          ("r" "Read" entry (file+headline "~/capture/someday.org" "Read") "** %^{title}\n %^{author}" :empty-lines 1)
          ("s" "Subject" entry (file+headline "~/capture/someday.org" "Write"), "** %^{subject}\n" :empty-lines 1)
          ("t" "Todo" entry (file+headline "~/capture/todo.org" "Tasks") "** TODO %?\n %i\n %a" :empty-lines 1)
          ("W" "Wishlist" plain (plain "~/capture/someday.org" "Wishlist") "** %^{thing}" :empty-lines 1)
          ("w" "Watch" entry (file+headline "~/capture/someday.org" "Watch") "** %^{movie title}\n %a" :empty-lines 1)))
...
Edit: now that i've moved to Arch Linux there is a package I can install instead

#+begin_src elisp :tangle no

  (setq org-ditaa-jar-path "~/projects/contrib/scripts/ditaa0_9/ditaa0_9.jar")
...
(setq org-plantuml-jar-path
      (expand-file-name "~/projects/contrib/scripts/plantuml-nodot.1.2023.7.jar"))
#+end_src
...
** Undo
If you need to clean up files that have already been saved incorrectly, cd into that dir and:

#+begin_example bash

find -type f -name '*.~undo-tree~' -delete
...
#+begin_src elisp :tangle no
  ...
  (use-package org-ai
    :straight (org-ai
               ...
               :host github
               :repo "rksm/org-ai"
               :local-repo "~/projects/contrib/org-ai"
...
(use-package aidermacs
  ...
  :init
  ;; Set environment variables for API keys before running Aider using auth-source
  ...
  (aidermacs-use-architect-mode t)  ;; Enable architect mode
  (aidermacs-default-model "grok")  ;; Use Claude Sonnet as default
  ;; Add custom model mappings
  (aidermacs-model-aliases '(("sonnet" . "claude-3-5-sonnet-20241022")
                             ("gpt4" . "gpt-4o")
                             ("gemini" . "gemini-pro"))))

;; Add Grok support via gptel
(use-package gptel
  :ensure t
  :config
  (setq gptel-api-key
        (lambda ()
          (auth-source-pick-first-password
           :host "api.x.ai"
           :user "mindthelion")))
  (gptel-make-xai "Grok"  ;; Define Grok backend
      :key gptel-api-key
      :url "https://api.x.ai/v1/chat/completions")  ;; Adjust URL if xAI provides a REST API later
  :bind (("C-c g" . gptel-menu)))  ;; Bind to access Grok
...
#+begin_src elisp :tangle no
    ...
    (use-package whisper
     :straight (whisper
                ...
                :host github
                :repo "natrys/whisper"
                :local-repo "~/projects/contrib/whisper")
...
#+begin_src elisp :tangle sod.el
    ...
    (use-package org-ref
      :straight (org-ref
                  ...
                  :local-repo "~/projects/contrib/org-ref")
...
#+begin_src elisp :tangle sod.el
  ...
  (use-package helm-bibtex
    :straight (helm-bibtex
               ...
               :local-repo "~/projects/contrib/helm-bibtex")
    :custom
...
#+begin_src elisp :tangle no
  ...
  (use-package ox-reveal
    :straight (ox-reveal
               ...
               :local-repo "~/projects/contrib/ox-reveal"
               :files ("*.el"))
    :config
...
#+begin_src elisp :tangle no
    (add-to-list 'load-path "~/projects/contrib/emacs-reveal")
